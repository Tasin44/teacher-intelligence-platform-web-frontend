...
import { GroupStats } from '@/lib/api/grouping.api';

interface StudentGroupingStatsProps {
    stats: GroupStats | null;
}

const StudentGroupingStats = ({ stats }: StudentGroupingStatsProps) => {
    return (
        <div className="bg-[#1E2130] p-4.5 rounded-xl border border-[#2A2D3A] grid grid-cols-2 md:grid-cols-4 gap-4 divide-y md:divide-y-0 md:divide-x divide-[#2A2D3A]/60 text-center" id="summary-bar-card">
            <div>
                ...
                <strong className="text-lg font-bold text-slate-100 mt-1 block font-mono">{stats?.total_students || 0}</strong>
            </div>
            <div className="pt-2.5 md:pt-0">
                ...
                <strong className="text-lg font-bold text-accent-orange mt-1 block font-mono">{stats?.total_groups || 0}</strong>
            </div>
            <div className="pt-2.5 md:pt-0">
                <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest block">Avg Group Size</span>
                <strong className="text-lg font-bold text-slate-100 mt-1 block font-mono">{stats?.avg_group_size || 0} Students</strong>
            </div>
            <div className="pt-2.5 md:pt-0">
                <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest block">Diagnostic Cycle</span>
                <strong className="text-xs font-semibold text-emerald-400 mt-2 block uppercase tracking-wide">
                    {stats?.last_group_formed ? new Date(stats.last_group_formed).toLocaleDateString() : 'N/A'}
                </strong>
            </div>
        </div>
    );
